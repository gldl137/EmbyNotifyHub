#!/usr/bin/env bash
# =============================================================================
# 启动后端 (FastAPI/Uvicorn, 端口 7000, 托管 backend/static 前端)
...
# 环境变量: BACKEND_PORT (默认 7000), DATA_DIR (默认 app/data)
# =============================================================================
set -euo pipefail
...
if [ -z "$PYTHON_BIN" ]; then
  err "未找到 python3 或 python，请先安装 Python 3.12 (python:3.12-bookworm)。"
  exit 1
fi
...
if [ -d "$BACKEND_DIR/static" ] && [ -f "$BACKEND_DIR/static/index.html" ]; then
  log "前端静态目录: 存在 ($BACKEND_DIR/static)"
else
  warn "前端静态目录缺失 ($BACKEND_DIR/static/index.html 不存在)，请先运行 bash scripts/build-frontend.sh"
fi
...
# 运行时 PYTHONPATH: 指向 PIP_TARGET_DIR (确保 uvicorn 能找到依赖)
RUN_PYTHONPATH="$PIP_TARGET_DIR"

CMD=(python3 -m uvicorn app.main:app --host 0.0.0.0 --port "$BACKEND_PORT")

if [ "$DAEMON" -eq 1 ]; then
  mkdir -p "$APP_DIR/.run"
  cd "$BACKEND_DIR"
  if [ -n "$RUN_PYTHONPATH" ]; then
    PYTHONPATH="$RUN_PYTHONPATH" DATA_DIR="$DATA_DIR" "${CMD[@]}" >>"$APP_DIR/.run/backend.log" 2>&1 &
  else
    DATA_DIR="$DATA_DIR" "${CMD[@]}" >>"$APP_DIR/.run/backend.log" 2>&1 &
  fi
