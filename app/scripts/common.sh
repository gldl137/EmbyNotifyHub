#!/usr/bin/env bash
# =============================================================================
# EmbyNotifyHub 公共函数库 (被 start-backend.sh / start-frontend.sh / build-frontend.sh 引用)
# =============================================================================
set -euo pipefail
...
#   - 本地测试 (项目不在 /app): backend/pip-packages (可见目录, 不污染本机 Python)
...
if [ -n "${PIP_TARGET_DIR:-}" ]; then
  ...
  PIP_TARGET_DIR="$BACKEND_DIR/pip-packages"
fi
...
# 判断关键依赖 (fastapi) 是否已安装 (在 PIP_TARGET_DIR 或系统路径中均可)
py_deps_ready() {
  local py_path=""
  if [ -n "$PIP_TARGET_DIR" ]; then
  ...
  if [ -n "$py_path" ]; then
    PYTHONPATH="$py_path" pyrun -c "import fastapi" 2>/dev/null
  else
    pyrun -c "import fastapi" 2>/dev/null
  fi
}
